...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (findError) {
      logServerError("admin/suspend-agent find", findError);
    ...
    const { data: updatedAgent, error: updateError } = await supabase
      ...
      .update({
        ...
        account_locked_reason: "Suspended by administrator"
      })
      .eq("id", agent.id)
      .select(`
        agent_id,
    ...
    if (updateError) {
      logServerError("admin/suspend-agent update", updateError);
      return sendError(res, 500, "Unable to suspend agent.");
    }
    ...
    logServerError("admin/suspend-agent", error);
    return sendError(res, 500, "Unable to suspend agent.");
  }
};
